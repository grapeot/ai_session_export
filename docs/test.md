...
| `test_render_markdown_second_mind_shape` | Frontmatter ordering, JSON-quoted string values, `message_count`, CJK title in both frontmatter and `H1`, and the `## User` / `## Assistant` body for a session with no `project_directory` or `models_used`. |
| `test_render_markdown_opencode_shape` | `project_directory` and `models_used` frontmatter lines are emitted only when populated; body shape for a session that has both. |
